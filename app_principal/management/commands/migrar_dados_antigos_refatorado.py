        ...
        for csv_path in csv_files:
            ...
            with csv_path.open("r", encoding="utf-8-sig", newline="") as f:
                ...
                for i, row in enumerate(reader, start=2):
                    ...
                    # Company (chave pelo CNPJ) + atualização de dados auxiliares
                    company, _ = Company.objects.get_or_create(cnpj=cnpj, defaults={
                        "name": company_name, "email": email, "phone": phone, "address": address
                    })
                    updates = {}
                    ...
                    # Award (chave: tender + company + item)
                    ...
                    if created:
                        ...
                        if award.qty_awarded != qty:
                            award.qty_awarded = qty; changed = True
                        if award.unit_price != unit_price:
                            award.unit_price = unit_price; changed = True
                        if award.total_price != total_price:
                            award.total_price = total_price; changed = True
                        if changed:
